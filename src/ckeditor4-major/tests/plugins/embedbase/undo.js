...
bender.test({
	...
	'test undo and redo after edition': function () {
		...
		editor.once('dialogHide', function () {
			resume(function () {
				assert.areSame('//after/edition', objToArray(editor.widgets.instances)[0].data.url, 'widget url after edition');
		...
		bot.setData('<p>foo</p><div data-oembed-url="//before/edition"><p>url:%2F%2Fbefore%2Fedition</p></div><p>&nbsp;</p>', function () {
			objToArray(editor.widgets.instances)[0].focus();
			editor.resetUndo();
	...
	'test undo and redo after creation and edition': function () {
		...
		this.listeners.push(editor.on('dialogHide', function () {
			...
			resume(function () {
				assert.areSame(1, objToArray(editor.widgets.instances).length, '1 widget after creation and edition');
				assert.areSame(urls[1], objToArray(editor.widgets.instances)[0].data.url, 'widget url after edition');
				assertCommands(editor, true, false, 'after edition');

				editor.execCommand('undo');
				assert.areSame(1, objToArray(editor.widgets.instances).length, '1 widget after 1st undo');
				assert.areSame(urls[0], objToArray(editor.widgets.instances)[0].data.url, 'widget url after 1st undo');
				assertCommands(editor, true, true, 'after 1st undo');

				editor.execCommand('undo');
				assert.areSame(0, objToArray(editor.widgets.instances).length, '0 widgets after 2nd undo');
				assertCommands(editor, false, true, 'after 2nd undo');
				assert.areSame('<p>foo</p>', editor.getData(), 'data after 2nd undo');

				editor.execCommand('redo');
				assert.areSame(1, objToArray(editor.widgets.instances).length, '1 widget after 1st redo');
				assert.areSame(urls[0], objToArray(editor.widgets.instances)[0].data.url, 'widget url after 1st redo');
				assertCommands(editor, true, true, 'after 1st redo');

				editor.execCommand('redo');
				assert.areSame(1, objToArray(editor.widgets.instances).length, '1 widget after 2nd redo');
				assert.areSame(urls[1], objToArray(editor.widgets.instances)[0].data.url, 'widget url after 2nd redo');
