...
window.rangy = function () {
	...
	function N() {
		if (!c.initialized) {
			...
			if ((p = L(document, "body") ? document.body : document.getElementsByTagName("body")[0]) && k(p, "createTextRange")) {
				p = p.createTextRange();
				if (z(p)) w = true
			}
	...
	var g = ["startContainer", "startOffset", "endContainer", "endOffset", "collapsed", "commonAncestorContainer", "START_TO_START", "START_TO_END", "END_TO_START", "END_TO_END"],
	...
	c.warn = function (p) {
		p = "Rangy warning: " + p;
		if (c.config.alertOnWarn) window.alert(p); else typeof window.console != "undefined" && typeof window.console.log != "undefined" && window.console.log(p)
	};
	...
	if (typeof window == "undefined") C("No window found"); else if (typeof document == "undefined") C("No document found"); else {
		k(document, "addEventListener") && document.addEventListener("DOMContentLoaded", q, false);
		if (k(window, "addEventListener")) window.addEventListener("load",
			q, false); else k(window, "attachEvent") ? window.attachEvent("onload", q) : C("Window does not have required addEventListener or attachEvent method");
		return c
	}
}();
rangy.createModule("DomUtil", function (k, L) {
	...
	function N(c, f) {
		var j = f.nextSibling, r = f.parentNode;
	...
	var A = k.util;
	A.areHostMethods(document, ["createDocumentFragment", "createElement", "createTextNode"]) || L.fail("document missing a Node creation method");
	A.isHostMethod(document, "getElementsByTagName") || L.fail("document missing getElementsByTagName method");
	var q = document.createElement("div");
	A.areHostMethods(q, ["insertBefore", "appendChild", "cloneNode"]) || L.fail("Incomplete Element implementation");
	A.isHostProperty(q, "innerHTML") || L.fail("Element is missing innerHTML property");
	q = document.createTextNode("test");
	A.areHostMethods(q, ["splitText", "deleteData", "insertData", "appendData", "cloneNode"]) || L.fail("Incomplete Text Node implementation");
	var v = function (c, f) {
	...
	n.prototype = {
		_current: null, hasNext: function () {
			...
			if (this._current) if (f = c.firstChild) this._next = f; else {
	...
	k.dom = {
		arrayContains: v, isHtmlNamespace: function (c) {
		...
		}, getNodeIndex: J, getNodeLength: function (c) {
			var f;
			return C(c) ? c.length : (f = c.childNodes) ? f.length : 0
		}, getCommonAncestor: K, isAncestorOf: function (c, f, j) {
			for (f = j ? f : f.parentNode; f;) if (f === c) return true; else f = f.parentNode;
			return false
			...
			c = O(c);
			if (typeof c.defaultView != "undefined") return c.defaultView; else if (typeof c.parentWindow != "undefined") return c.parentWindow; else throw Error("Cannot get a window object for node");
			...
			if (typeof c.contentDocument != "undefined") return c.contentDocument; else if (typeof c.contentWindow != "undefined") return c.contentWindow.document; else throw Error("getIframeWindow: No Document object found for iframe element");
		}, getBody: function (c) {
			...
			var M;
			if (c == j) return f === r ? 0 : f < r ? -1 : 1; else if (M = z(j,
				c, true)) return f <= J(M) ? -1 : 1; else if (M = z(c, j, true)) return J(M) < r ? -1 : 1; else {
				f = K(c, j);
				c = c === f ? f : z(c, f, true);
				j = j === f ? f : z(j, f, true);
				if (c === j) throw Error("comparePoints got to case 4 and childA and childB are the same!"); else {
					for (f = f.firstChild; f;) {
						if (f === c) return -1; else if (f === j) return 1;
						f = f.nextSibling
					}
					throw Error("Should not be here!");
				}
			}
		}, inspectNode: g, fragmentFromNodeChildren: function (c) {
			for (var f = O(c).createDocumentFragment(), j; j = c.firstChild;) f.appendChild(j);
			return f
...
rangy.createModule("DomRange", function (k) {
	function L(a, d) {
		return a.nodeType != 3 && (l.isAncestorOf(a, d.startContainer, true) || l.isAncestorOf(a, d.endContainer, true))
	}
	...
	function K(a, d, h) {
		if (d = a._listeners[d]) for (var o = 0, G = d.length; o < G; ++o) d[o].call(a, {target: a, args: h})
	}
	...
	function N(a, d, h) {
		...
		if (l.isCharacterDataNode(d)) h ==
		d.length ? l.insertAfter(a, d) : d.parentNode.insertBefore(a, h == 0 ? d : l.splitDataNode(d, h)); else h >= d.childNodes.length ? d.appendChild(a) : d.insertBefore(a, d.childNodes[h]);
		return o
	}
	...
	function g(a, d, h) {
		var o, G;
		for (h = h || {stop: false}; o = a.next();) if (a.isPartiallySelectedSubtree()) if (d(o) ===
			false) {
			h.stop = true;
			return
		} else {
			...
			o.detach(true);
			if (h.stop) return
		} else for (o = l.createIterator(o); G = o.next();) if (d(G) === false) {
			h.stop = true;
			return
		}
	}
	...
	function n(a) {
		for (var d; a.next();) if (a.isPartiallySelectedSubtree()) {
			d = a.getSubtreeIterator();
			n(d);
			d.detach(true)
		} else a.remove()
	...
	function A(a) {
		return "[" + (typeof a.getName == "undefined" ? "Range" : a.getName()) + "(" + l.inspectNode(a.startContainer) + ":" + a.startOffset + ", " + l.inspectNode(a.endContainer) + ":" + a.endOffset + ")]"
	}
	...
	function q(a, d) {
		...
		if (!a.collapsed) {
			...
			if (this.sc === this.ec && l.isCharacterDataNode(this.sc)) {
				...
				this._first = this._next = this.sc === h && !l.isCharacterDataNode(this.sc) ? this.sc.childNodes[this.so] : l.getClosestAncestorIn(this.sc, h, true);
				this._last = this.ec === h && !l.isCharacterDataNode(this.ec) ? this.ec.childNodes[this.eo - 1] : l.getClosestAncestorIn(this.ec,
					h, true)
			}
		}
	}
	...
	function p(a, d) {
		if (d < 0 || d > (l.isCharacterDataNode(a) ? a.length : a.childNodes.length)) throw new Q("INDEX_SIZE_ERR");
	}
	...
	function F(a) {
		r(a);
		if (!l.arrayContains(Y, a.startContainer.nodeType) && !e(a.startContainer, true) || !l.arrayContains(Y, a.endContainer.nodeType) && !e(a.endContainer, true) || !(a.startOffset <= (l.isCharacterDataNode(a.startContainer) ?
	...
	function X(a, d) {
		return function () {
			F(this);
			var h = this.startContainer, o = this.startOffset, G = this.commonAncestorContainer, U = new q(this, true);
			if (h !== G) {
	...
	function aa(a, d, h) {
		...
		function G(m, s, y) {
			var H = m.endContainer, R = m.endOffset;
		...
		function U(m, s, y) {
			var H = m.startContainer, R = m.startOffset;
		...
		k.util.extend(a.prototype, {
			...
			collapse: function (m) {
				F(this);
				m ? d(this, this.startContainer,
					this.startOffset, this.startContainer, this.startOffset) : d(this, this.endContainer, this.endOffset, this.endContainer, this.endOffset)
			},
			...
			canSurroundContents: function () {
				...
				w(this.endContainer);
				var m = new q(this, true), s = m._first && L(m._first, this) ||
					m._last && L(m._last, this);
				m.detach();
				return !s
			},
			detach: function () {
			...
			splitBoundaries: function () {
				F(this);
				var m = this.startContainer, s = this.startOffset, y = this.endContainer, H = this.endOffset,
					R = m === y;
				l.isCharacterDataNode(y) && H > 0 && H < y.length && l.splitDataNode(y, H);
			...
			normalizeBoundaries: function () {
				F(this);
				var m = this.startContainer, s = this.startOffset, y =
					this.endContainer, H = this.endOffset, R = function (V) {
					var S = V.nextSibling;
				...
				if (l.isCharacterDataNode(y)) y.length == H && R(y); else {
					if (H > 0) (fa = y.childNodes[H - 1]) &&
					l.isCharacterDataNode(fa) && R(fa);
					fa = !this.collapsed
				}
				if (fa) if (l.isCharacterDataNode(m)) s == 0 && qa(m); else {
					if (s < m.childNodes.length) (R = m.childNodes[s]) && l.isCharacterDataNode(R) && qa(R)
				} else {
					m = y;
					s = H
				}
			...
			collapseToPoint: function (m, s) {
				...
				j(m, true);
				p(m, s);
				if (m !== this.startContainer || s !== this.startOffset || m !== this.endContainer || s !== this.endOffset) d(this, m, s, m, s)
			}
		});
		Z(a)
	}
	...
	function ga(a) {
		...
		a.commonAncestorContainer = a.collapsed ?
			a.startContainer : l.getCommonAncestor(a.startContainer, a.endContainer)
	}

	function ba(a, d, h, o, G) {
		var U = a.startContainer !== d || a.startOffset !== h, m = a.endContainer !== o || a.endOffset !== G;
		a.startContainer = d;
		a.startOffset = h;
		a.endContainer = o;
		a.endOffset = G;
	...
	var l = k.dom, D = l.DomPosition,
		Q = k.DOMException;
	q.prototype = {
		_current: null, _next: null, _first: null, _last: null, isSingleCharacterDataNode: false, reset: function () {
			this._current = null;
			...
			if (a) {
				...
				if (l.isCharacterDataNode(a) && this.clonePartiallySelectedTextNodes) {
					...
					if (this._current === this.sc) (a = a.cloneNode(true)).deleteData(0,
						this.so)
				}
			}
			return a
		}, remove: function () {
			var a = this._current, d, h;
			...
			if (this.isSingleCharacterDataNode) {
				...
				var d = this._current, h = d, o = 0, G = d, U = l.getNodeLength(d);
	...
	var ea = [1, 3, 4, 5, 7, 8, 10], Y = [2, 9, 11], ha = [1, 3, 4, 5, 7, 8, 10, 11], $ = [1, 3, 4, 5, 7, 8],
		b = l.getRootContainer, e = f([9, 11]), i = f([5, 6, 10, 12]), E = f([6, 10, 12]),
		I = document.createElement("style"), P = false;
	try {
		I.innerHTML = "<b>x</b>";
	...
	var ia = ["startContainer", "startOffset", "endContainer", "endOffset", "collapsed", "commonAncestorContainer"],
		ja = 0, la = 1, ra = 2, ma = 3, na = 0, oa = 1, pa = 2, ka = 3;
	W.prototype = {
		attachListener: function (a, d) {
			...
			var h = a == ma || a == ja ? "start" : "end", o = a == la || a == ja ? "start" : "end";
			return l.comparePoints(this[h + "Container"], this[h + "Offset"], d[o + "Container"], d[o + "Offset"])
		}, insertNode: function (a) {
			F(this);
			M(a, ha);
			w(this.startContainer);
			if (l.isAncestorOf(a, this.startContainer, true)) throw new Q("HIERARCHY_REQUEST_ERR");
			this.setStartBefore(N(a,
				this.startContainer, this.startOffset))
		}, cloneContents: function () {
			F(this);
			var a, d;
			if (this.collapsed) return J(this).createDocumentFragment(); else {
			...
			w(this.startContainer);
			w(this.endContainer);
			var a = new q(this, true), d = a._first && L(a._first, this) || a._last && L(a._last, this);
			a.detach();
			return !d
			...
			if (a === this.endContainer && l.isCharacterDataNode(a)) return a.nodeType == 3 || a.nodeType == 4 ? a.data.slice(this.startOffset, this.endOffset) : ""; else {
				var d = [];
				a = new q(this, true);
				g(a, function (h) {
			...
			F(this);
			var d = a.parentNode, h = l.getNodeIndex(a);
			...
			if (l.comparePoints(a, d, this.startContainer, this.startOffset) < 0) return -1; else if (l.comparePoints(a, d, this.endContainer, this.endOffset) > 0) return 1;
			return 0
		}, createContextualFragment: P ? function (a) {
			var d = this.startContainer, h = l.getDocument(d);
			if (!d) throw new Q("INVALID_STATE_ERR");
			var o = null;
			if (d.nodeType == 1) o = d; else if (l.isCharacterDataNode(d)) o = l.parentElement(d);
			o = o === null || o.nodeName == "HTML" && l.isHtmlNamespace(l.getDocument(o).documentElement) &&
			l.isHtmlNamespace(o) ? h.createElement("body") : o.cloneNode(false);
			o.innerHTML = a;
			...
			var h = a.parentNode, o = l.getNodeIndex(a);
			B(h, "NOT_FOUND_ERR");
			...
			B(a, "HIERARCHY_REQUEST_ERR");
			u(a, this.startContainer);
			return l.comparePoints(a, d, this.startContainer, this.startOffset) >= 0 && l.comparePoints(a, d, this.endContainer, this.endOffset) <= 0
		}, intersectsRange: function (a, d) {
			F(this);
			if (J(a) != J(this)) throw new Q("WRONG_DOCUMENT_ERR");
			var h = l.comparePoints(this.startContainer,
					this.startOffset, a.endContainer, a.endOffset),
				o = l.comparePoints(this.endContainer, this.endOffset, a.startContainer, a.startOffset);
			return d ? h <= 0 && o >= 0 : h < 0 && o > 0
		}, intersection: function (a) {
			if (this.intersectsRange(a)) {
				var d = l.comparePoints(this.startContainer, this.startOffset, a.startContainer, a.startOffset),
					h = l.comparePoints(this.endContainer, this.endOffset, a.endContainer, a.endOffset),
					o = this.cloneRange();
		...
		union: function (a) {
			if (this.intersectsRange(a, true)) {
				...
				l.comparePoints(a.startContainer, a.startOffset, this.startContainer, this.startOffset) == -1 && d.setStart(a.startContainer, a.startOffset);
				l.comparePoints(a.endContainer, a.endOffset, this.endContainer, this.endOffset) == 1 && d.setEnd(a.endContainer, a.endOffset);
				return d
			...
			return this.comparePoint(a,
				0) >= 0 && this.comparePoint(a, l.getNodeLength(a)) <= 0
...
rangy.createModule("WrappedRange", function (k) {
	...
	function J(g, n) {
		var t, x, A = g.offset, q = z.getDocument(g.node), v = q.body.createTextRange(),
	...
	var K, z = k.dom, C = z.DomPosition, N = k.DomRange;
	if (k.features.implementsDomRange && (!k.features.implementsTextRange || !k.config.preferTextRange)) {
		(function () {
			...
			N.createPrototypeRange(K, function (f, j, r, M, p) {
			...
			x.setEnd(q, 4);
			x.setStart(q, 2);
			n.compareBoundaryPoints = v.compareBoundaryPoints(v.START_TO_END, x) == -1 & v.compareBoundaryPoints(v.END_TO_START, x) == 1 ? function (f, j) {
				j = j.nativeRange || j;
			...
			if (k.util.isHostMethod(v, "createContextualFragment")) n.createContextualFragment = function (f) {
		...
		K.prototype.refresh = function () {
			...
			if (this.textRange.compareEndPoints("StartToEnd", this.textRange) == 0) n = g = L(this.textRange, n, true, true); else {
	...
	if (k.features.implementsTextRange) K.rangeToTextRange = function (g) {
		if (g.collapsed) return J(new C(g.startContainer, g.startOffset), true); else {
			var n = J(new C(g.startContainer, g.startOffset), true), t = J(new C(g.endContainer, g.endOffset), false);
	...
	k.addCreateMissingNativeApiListener(function (g) {
		g = g.document;
		if (typeof g.createRange == "undefined") g.createRange = function () {
...
rangy.createModule("WrappedSelection", function (k, L) {
	...
	function N(b) {
		...
		} else if (b instanceof r) e = b.nativeRange; else if (k.features.implementsDomRange && b instanceof c.getWindow(b.startContainer).Range) e = b;
		return e
	}

	function O(b) {
		var e = b.getNodes(), i;
		a:if (!e.length || e[0].nodeType != 1) i = false; else {
			i = 1;
			for (var E = e.length; i < E; ++i) if (!c.isAncestorOf(e[0],
				e[i])) {
				i = false;
				break a
			}
		...
		if (!i) throw Error("getSingleElementFromRange: range " + b.inspect() + " did not consist of a single element");
		return e[0]
	}
	...
	function n(b) {
		b._ranges.length = 0;
		if (b.docSelection.type == "None") C(b); else {
			var e = b.docSelection.createRange();
			if (e && typeof e.text != "undefined") g(b, e); else {
				b.rangeCount = e.length;
				for (var i, E = c.getDocument(e.item(0)), I = 0; I < b.rangeCount; ++I) {
					i = k.createRange(E);
	...
	function t(b, e) {
		var i = b.docSelection.createRange(), E = O(e), I = c.getDocument(i.item(0));
	...
	function q(b, e) {
		if (b.anchorNode && c.getDocument(b.anchorNode) !== c.getDocument(e)) throw new M("WRONG_DOCUMENT_ERR");
	}

	function v(b) {
		var e =
				[], i = new p(b.anchorNode, b.anchorOffset), E = new p(b.focusNode, b.focusOffset),
			I = typeof b.getName == "function" ? b.getName() : "Selection";
		if (typeof b.rangeCount != "undefined") for (var P = 0, ca = b.rangeCount; P < ca; ++P) e[P] = j.inspect(b.getRangeAt(P));
		return "[" + I + "(Ranges: " + e.join(", ") + ")(anchor: " + i.inspect() + ", focus: " + E.inspect() + "]"
	}

	k.requireModules(["DomUtil", "DomRange", "WrappedRange"]);
	k.config.checkSelectionRanges = true;
	var c = k.dom, f = k.util, j = k.DomRange, r = k.WrappedRange, M = k.DOMException, p = c.DomPosition, u,
		w, B = k.util.isHostMethod(window, "getSelection"), F = k.util.isHostObject(document, "selection"),
	...
	if (W) {
		...
		k.isSelectionValid = function (b) {
			b = (b || window).document;
			var e = b.selection;
			return e.type != "None" || c.getDocument(e.createRange().parentElement()) == b
		}
	} else if (B) {
		u = J;
	...
	B = u();
	var da = k.createNativeRange(document), Z = c.getBody(document),
		X = f.areHostObjects(B, f.areHostProperties(B, ["anchorOffset", "focusOffset"]));
	...
	var ba = false, T = true;
	f.areHostMethods(B, ["addRange", "getRangeAt", "removeAllRanges"]) && typeof B.rangeCount == "number" && k.features.implementsDomRange && function () {
		...
		e.close();
		var i = c.getIframeWindow(b).getSelection(), E = e.documentElement.lastChild.firstChild;
		e = e.createRange();
		e.setStart(E, 1);
		e.collapse(true);
		i.addRange(e);
	...
	var Q;
	if (f.isHostMethod(B, "getRangeAt")) Q = function (b, e) {
		try {
			return b.getRangeAt(e)
		} catch (i) {
			return null
		}
	}; else if (X) Q = function (b) {
		var e = c.getDocument(b.anchorNode);
		e = k.createRange(e);
		e.setStart(b.anchorNode,
			b.anchorOffset);
	...
	k.getSelection = function (b) {
		b = b || window;
		var e = b._rangySelection, i = u(b), E = F ? K(b) : null;
		if (e) {
			e.nativeSelection = i;
			e.docSelection = E;
			e.refresh(b)
	...
	D = x.prototype;
	if (!W && X && f.areHostMethods(B, ["removeAllRanges",
		"addRange"])) {
		...
		D.addRange = ga ? function (b, e) {
			if (l && F && this.docSelection.type == "Control") t(this, b); else if (e && aa) ea(this, b); else {
				var i;
				if (ba) i = this.rangeCount; else {
				...
				if (this.rangeCount == i + 1) {
					if (k.config.checkSelectionRanges) if ((i = Q(this.nativeSelection, this.rangeCount - 1)) && !j.rangesEqual(i, b)) b = new r(i);
			...
			if (e && aa) ea(this, b); else {
				this.nativeSelection.addRange(N(b));
				this.refresh()
			}
		};
		D.setRanges = function (b) {
			if (l && b.length > 1) A(this, b); else {
				this.removeAllRanges();
				for (var e = 0, i = b.length; e < i; ++e) this.addRange(b[e])
			}
		}
	} else if (f.isHostMethod(B,
		"empty") && f.isHostMethod(da, "select") && l && W) {
		D.removeAllRanges = function () {
			try {
				this.docSelection.empty();
				if (this.docSelection.type != "None") {
					var b;
					if (this.anchorNode) b = c.getDocument(this.anchorNode); else if (this.docSelection.type == "Control") {
		...
		D.addRange = function (b) {
			if (this.docSelection.type == "Control") t(this, b); else {
		...
		D.setRanges = function (b) {
			this.removeAllRanges();
			var e = b.length;
			if (e > 1) A(this, b); else e && this.addRange(b[0])
		}
	} else {
	...
	D.getRangeAt = function (b) {
		if (b < 0 || b >= this.rangeCount) throw new M("INDEX_SIZE_ERR"); else return this._ranges[b]
	};
	var Y;
	if (W) Y = function (b) {
		var e;
		if (k.isSelectionValid(b.win)) e = b.docSelection.createRange(); else {
			e = c.getBody(b.win.document).createTextRange();
			e.collapse(true)
		}
		if (b.docSelection.type == "Control") n(b); else e && typeof e.text != "undefined" ? g(b, e) : C(b)
	}; else if (f.isHostMethod(B, "getRangeAt") && typeof B.rangeCount == "number") Y = function (b) {
		if (l && F && b.docSelection.type == "Control") n(b); else {
			b._ranges.length = b.rangeCount = b.nativeSelection.rangeCount;
			if (b.rangeCount) {
				for (var e = 0, i = b.rangeCount; e < i; ++e) b._ranges[e] = new k.WrappedRange(b.nativeSelection.getRangeAt(e));
				z(b, b._ranges[b.rangeCount - 1], $(b.nativeSelection));
				b.isCollapsed = w(b)
			} else C(b)
		}
	}; else if (X &&
		typeof B.isCollapsed == "boolean" && typeof da.collapsed == "boolean" && k.features.implementsDomRange) Y = function (b) {
		var e;
		e = b.nativeSelection;
		if (e.anchorNode) {
			e = Q(e, 0);
	...
	var ha = function (b, e) {
		var i = b.getAllRanges(), E = false;
		b.removeAllRanges();
		for (var I = 0, P = i.length; I < P; ++I) if (E || e !== i[I]) b.addRange(i[I]); else E = true;
		b.rangeCount || C(b)
	};
	D.removeRange = l ? function (b) {
		if (this.docSelection.type == "Control") {
			...
			i = c.getBody(i).createControlRange();
			for (var E,
					 I = false, P = 0, ca = e.length; P < ca; ++P) {
				E = e.item(P);
				if (E !== b || I) i.add(e.item(P)); else I = true
			}
			i.select();
			n(this)
		} else ha(this, b)
	...
	if (!W && X && k.features.implementsDomRange) {
		$ = function (b) {
			var e = false;
			if (b.anchorNode) e = c.comparePoints(b.anchorNode, b.anchorOffset, b.focusNode, b.focusOffset) == 1;
			return e
		};
		D.isBackwards = function () {
			return $(this)
		}
	} else $ = D.isBackwards = function () {
		return false
	};
	D.toString = function () {
		for (var b = [], e = 0, i = this.rangeCount; e < i; ++e) b[e] = "" + this._ranges[e];
		return b.join("")
	};
	...
	D.containsNode = function (b, e) {
		for (var i = 0, E = this._ranges.length; i < E; ++i) if (this._ranges[i].containsNode(b, e)) return true;
		return false
	};
	D.toHtml = function () {
		var b = "";
		if (this.rangeCount) {
			...
			for (var e = 0, i = this._ranges.length; e < i; ++e) b.appendChild(this._ranges[e].cloneContents());
			b = b.innerHTML
		}
		return b
	};
