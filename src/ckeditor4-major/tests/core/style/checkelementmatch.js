	...
	bender.test({
		...
		'test styles color attribute normalization': function () {
			var styleItalic = new CKEDITOR.style({element: 'h2', attributes: {'style': 'font-style:italic;'}}),
				styleSubtitle = new CKEDITOR.style({
					element: 'h3',
					attributes: {'style': 'color:#a1a2a3;font-style:italic;'}
				}),
				styleSpecialContainer = new CKEDITOR.style({
					element: 'div',
					attributes: {'style': 'background:#eee;border:1px solid #ccc;padding:5px 10px;'}
				}),
		...
		'test styles font-family ignore quotes': function () {
			var styleFontMatch = new CKEDITOR.style({
					element: 'span',
					attributes: {'style': 'font-family:Univers LT Std;'}
				}),
