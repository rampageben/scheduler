...
addBookmarkTCs(tcs, {
	...
	'element selection': {
		...
		'b 2': ['x.y<i>b</i>z', {sc: '#y', so: 1, ec: '#z', eo: 0}, {sc: 'root', so: 3, ec: 'root', eo: 4}, {
			sc: 'root',
			so: 2,
			ec: 'root',
			eo: 3
		...
		'd': ['<u>x<i>d</i><b>y</b></u>', {sc: '#x', so: 1, ec: 'u', eo: 2}, {sc: 'u', so: 2, ec: 'u', eo: 3}, {
			sc: 'u',
			so: 1,
			ec: 'u',
			eo: 2
	...
	'text selection': {
		...
		'ab 2': ['<i>a</i><u>b</u>', {sc: '#a', so: 1, ec: '#b', eo: 0}, {sc: 'i', so: 2, ec: 'u', eo: 0}, {
			sc: 'i',
			so: 1,
			ec: 'u',
			eo: 0
		...
		'#10301 1': ['<p>a.b<i>c</i>d.e</p>', {sc: '#e', so: 0, ec: '#e', eo: 1}, {
			sc: 'p',
			so: 5,
			ec: 'p',
			eo: 6
...
addBookmark2TCs(tcs, {
	...
	'collapsed in empty text nodes': {
		...
		'<i>a</i>(foo)(bar)<u>b</u> - range in bar': ['<i>a</i>(foo)(bar)<u>b</u>', {sc: '(bar)', so: 0}, {
			sc: 'root',
			so: 1
		}],
	...
	'collapsed in element': {
		...
		// <b>(foo)[<i>d]ef</i></b>
		'abcdef 1': ['<b>(foo)<i>def</i></b>', {sc: 'b', so: 1, ec: '#def', eo: 1}, {
			sc: 'b',
			so: 0,
			ec: '#def',
			eo: 1
		...
		'abcdef 2': ['<b>(foo)(bar)<i>def</i></b>', {sc: 'b', so: 2, ec: '#def', eo: 1}, {
			sc: 'b',
			so: 0,
			ec: '#def',
			eo: 1
	...
	'filling character sequence': {
		'tc 1': ['<p>abc</p><p><b>%def</b></p>', {sc: '#abc', so: 1, ec: '#def', eo: FCSLength + 2}, {
			sc: '#abc',
			so: 1,
			ec: '#def',
			eo: 2
		}],
		'tc 2': ['%abc<b>def</b>', {sc: '#abc', so: FCSLength + 1, ec: '#def', eo: 1}, {
			sc: '#abc',
			so: 1,
			ec: '#def',
			eo: 1
		...
		'tc 3': ['<b>a</b>%<b>c</b>d<i>e</i>', {sc: 'root', so: 2, ec: '#e', eo: 1}, {
			sc: 'root',
			so: 1,
			ec: '#e',
			eo: 1
		...
		'tc 6b': ['<b>%(foo)(bar)<i>def</i></b>', {sc: 'b', so: 2, ec: '#def', eo: 1}, {
			sc: 'b',
			so: 0,
			ec: '#def',
			eo: 1
		}],
		'tc 6c': ['<b>%(foo)(bar)<i>def</i></b>', {sc: '(bar)', so: 0, ec: '#def', eo: 1}, {
			sc: 'b',
			so: 0,
			ec: '#def',
			eo: 1
		...
		// between foo and bar.
		'tc 6d': ['<b>abc%(foo)(bar)cba<i>def</i></b>', {sc: 'b', so: 3, ec: '#def', eo: 1}, {
			sc: '#abccba',
			so: 3,
			ec: '#def',
			eo: 1
